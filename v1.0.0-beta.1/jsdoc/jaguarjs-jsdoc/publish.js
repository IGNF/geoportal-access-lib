...
function buildNav(members) {
    ...
    //On fusionne tous les items pour tous les traiter en même temps dans la barre de navigation
    var allItems = [];
    allItems.push.apply(allItems, members.modules);
    allItems.push.apply(allItems, members.classes);
    allItems.push.apply(allItems, members.namespaces);

    //On trie le tableau pour classer les items par ordre alphabétique des alias
    allItems.sort(function(a, b) {
        return a.alias == b.alias ? 0 : +(a.alias > b.alias) || -1;
    });
   
     if (allItems.length) {
        _.each(allItems, function (v) {
            nav.push({
                type: 'class',
                longname: v.longname,
                alias: v.alias,

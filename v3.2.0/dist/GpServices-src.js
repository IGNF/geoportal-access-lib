...
var servicesDate = _package_json__WEBPACK_IMPORTED_MODULE_26__.date; // on declare les ns dans root global
...
Gp.Protocols = {};
Gp.Protocols.XHR = _Protocols_XHR__WEBPACK_IMPORTED_MODULE_23__["default"]; // Export services
...
Gp.Services.Config.TileMatrixSet = _Services_AutoConf_Response_model_TileMatrixSet__WEBPACK_IMPORTED_MODULE_19__["default"]; // Export IsoCurve

Gp.Services.IsoCurveResponse = _Services_ProcessIsoCurve_Response_model_ProcessIsoCurveResponse__WEBPACK_IMPORTED_MODULE_20__["default"]; // Export Route

Gp.Services.RouteResponse = _Services_Route_Response_model_RouteResponse__WEBPACK_IMPORTED_MODULE_21__["default"];
Gp.Services.Route = {};
Gp.Services.Route.RouteInstruction = _Services_Route_Response_model_RouteInstruction__WEBPACK_IMPORTED_MODULE_22__["default"]; // Export Erreurs et Outils
...
function Geocode(options_) {
  ...
  options.serverUrl = options.serverUrl || "https://wxs.ign.fr/calcul/geoportail/geocodage/rest/0.1/search"; // appel du constructeur par heritage
  ...
  this.options.query = options.query; // on definit l'index par defaut
  ...
  if (options.filters) {
    ...
    for (var i = 0; i < filter.length; i++) {
      var key = filter[i]; // on supprime les filtres vides
...
Geocode.prototype.buildRequest = function (error, success) {
  ...
  this.request = _Request_GeocodeRequestFactory__WEBPACK_IMPORTED_MODULE_4__["default"].build(options); // on teste si la requete a bien été construite !
...
GeocodeRequestREST.prototype = {
   ...
   */
  processRequestString: function processRequestString() {
    var request = ""; // Mapping des options avec le service de l'API REST
...
function GeocodeParamREST(options) {
  ...
  this.options = options || {}; // methode de geocodage

  if (!this.options.geocodeMethod || this.options.geocodeMethod !== "search" && this.options.geocodeMethod !== "reverse") {
    throw new Error("Error geocodeMethod not valid");
  }

  this.geocodeMethod = this.options.geocodeMethod; // mapping des options avec l'API REST
...
function ReverseGeocode(options_) {
  ...
  var options = this.patchOptionConvertor(options_);
  options.serverUrl = options.serverUrl || "https://wxs.ign.fr/calcul/geoportail/geocodage/rest/0.1/reverse"; // appel du constructeur par heritage
  ...
  this.options.searchGeometry = options.searchGeometry; // on definit l'index par defaut
...
function AutoComplete(options_) {
  ...
  var options = this.patchOptionConvertor(options_);
  options.serverUrl = options.serverUrl || "https://wxs.ign.fr/calcul/geoportail/geocodage/rest/0.1/completion"; // appel du constructeur par heritage
  ...
  this.options.text = options.text; // on definit des parametres par defaut
...
function ProcessIsoCurve(options) {
  ...
  this.options.srs = options.srs || "EPSG:4326";
  this.options.distanceUnit = options.distanceUnit || "m";
  this.options.timeUnit = options.timeUnit || "second"; // options depreciees
  ...
  this.options.holes = false; // Gestion du graphe
  ...
  this.options.outputFormat = "json"; // gestion de l'url du service par defaut
  // si l'url n'est pas renseignée, il faut utiliser les urls par defaut
...
ProcessIsoCurveRequest.prototype = {
  ...
  processRequestString: function processRequestString() {
    ...
    switch (this.mode) {
      ...
      case "POST":
        this.logger.trace("Process POST Request"); // creation du JSON
        ...
        postRequest.crs = this.settings.srs; // conversion en chaîne de caractères
  ...
  build: function build(options) {
    ...
    if (options.response) {
      if (options.rawResponse) {
        ...
        data = new _model_ProcessIsoCurveResponse__WEBPACK_IMPORTED_MODULE_3__["default"](); // remplissage de l'objet créé avec les attribtuts de la réponse du service
